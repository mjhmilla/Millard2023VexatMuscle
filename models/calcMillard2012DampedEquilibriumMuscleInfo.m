...
%             In addition the state is now fiberLengthAlongTendon instead 
%             of fiberLength at the suggestion of Ton Van den Bogert who 
%             suggests that this state variable is easier to integrate 
%             than fiberLength.
%
...
a     = clampActivation(activationState(2), ...
...
alphaLceMin  = muscleArchitecture.pennationAngleAtMinimumFiberLength;
...
calcFvInvDer = @(arg1, arg2)calcBezierYFcnXDerivative(arg1, ...
...
if(useElasticTendon == 0)
   ...
   fiberState = clampFiberStateAlongTendon(lceAT,dlceAT,lceATMin*1.5);
   ...
   %
   kfN = (a*DfalN_DlceN*fvN + DfpeN_DlceN);
   
   kf = kfN*(fiso/lceOpt);
   
   DfvN_DdlceN =  calcFvDer(dlceN, 1);
   DdlceN_Ddlce = 1/(lceOpt*dlceMaxN);
   dfN = (a*falN*DfvN_DdlceN);
   df  = fiso*dfN*DdlceN_Ddlce;
   dfAT= df*cosAlpha;
...
elseif(useElasticTendon == 1 )
    ...
    if(useFiberDamping == 0)
        ...
        %
        falN       =  calcFalDer(lceN, 0);%calcFalHACKDer(lceN, 0);
        DfalN_DlceN = calcFalDer(lceN, 1);%calcFalHACKDer(lceN, 1);
        ...
        if(useInitializationCode == 0)
            ...
            dlceN = calcFvInvDer(fvN,0);%calcFvInvHACKDer(fvN,0);       
        ...
        dlceN = dlce/(lceOpt*dlceMaxN);
        fvN    = calcFvDer(dlceN,0);%calcFvHACKDer(dlceN,0);
        ...
        DfvN_DdlceN =  calcFvDer(dlceN,1);%calcFvHACKDer(dlceN,1);
        DdlceN_Ddlce = 1/(lceOpt*dlceMaxN);
        dfN = (a*falN*DfvN_DdlceN);
        df  = fiso*dfN*DdlceN_Ddlce;        
    ...
    elseif(useFiberDamping == 1)        
        ...
        if(useInitializationCode == 0)
            ...
            %use the classic elastic tendon equations to get a very good
            %initial solution.        
            cosAlphaHack = max([cosAlpha,0.01]);
            aHack        = max([a,0.01]);
            falNHack      = max([falN,0.01]);

            
            dlceNNearMaxShortening = ...
                mean(normMuscleCurves.fiberForceVelocityCurve.xpts(:,1));
            xptsCol = ...
                size(normMuscleCurves.fiberForceVelocityCurve.xpts,2);
            dlceNNearMaxLengthening = ...
                mean(normMuscleCurves.fiberForceVelocityCurve.xpts(:,xptsCol));

            
            fvN    = ((ftN/cosAlphaHack) - fpeN) / (aHack*falNHack);
            dlceN = calcFvInvDer(fvN,0);%calcFvInvHACKDer(fvN,0);
            
            if(dlceN < dlceNNearMaxShortening)
                dlceN = dlceNNearMaxShortening;
            end
            if(dlceN > dlceNNearMaxLengthening)
                dlceN = dlceNNearMaxLengthening; 
            end
            %%
            %Evaluate dlce/dt to satisfy the damped equilibrium eqn.
            %%
...
mtInfo.muscleDynamicsInfo.normPassiveFiberForce     = ffpN;      % force                N
...
mtInfo.muscleDynamicsInfo.crossBridgeStiffness      = kf;
mtInfo.muscleDynamicsInfo.crossBridgeDamping        = df;

mtInfo.muscleDynamicsInfo.tendonStiffness           = kt;             % force/length         N/m
mtInfo.muscleDynamicsInfo.tendonDamping             = 0;
...
mtInfo.muscleDynamicsInfo.normActiveFiberForce      = ffaN;
mtInfo.muscleDynamicsInfo.normMuscleStiffness       = kfN;
