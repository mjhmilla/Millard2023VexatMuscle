...
switch flag_useElasticTendon
    ...
    case 1
        strCaption = 'Mean normalized stiffness coefficients (A.), mean normalized damping coefficients (B.), VAF (C.), and the bandwidth (D.) of linearity (coherence squared $> 0.67$) for models with elastic tendons. Here the proposed model has been fitted to Figure 12 of Kirsch et al. \cite{Kirsch1994MuscleImpedance}, while the experimental data from Kirsch et al. \cite{Kirsch1994MuscleImpedance} comes from Figures 9 and 10. Experimental data from Figure 12 from Kirsch et al. has not been included in this table because it would only contribute 1 entry and would overwrite values from Figures 9 and 10. The impedance experiments at each combination of perturbation amplitude and frequency have been evaluated at 10 different nominal forces linearly spaced between 2.5N and 11.5N. The normalized results presented in the table are the mean values of these ten simulations. The VAF is evaluated between the model and the spring-damper of best fit, rather than to the response of biological muscle (which was not published by Kirsch et al. \cite{Kirsch1994MuscleImpedance}. Finally, model values for the VAF (C.) and the bandwidth of linearity (D.) that are worse than those published by Kirsch et al. appear in bold font.';
...
for idxTrial=1:1:length(kbr1994Table.vaf.data)
  ...
  valKbr       = min(kbr1994Table.vaf.data(idxTrial).y)*100;
...
for idxTrial=1:1:length(kbr1994Table.coherenceSqFreqLb.data)
  ...
  if(valLb > round(valLbKbr+1))
      valLbStr = sprintf('\\textbf{%1.0f}',valLb);
  else
      valLbStr = sprintf('%1.0f',valLbKbr);    
  end
  valUbStr = sprintf('%1.0f',valUb);
  if(valUb < round( valUbKbr-1))
      valUbStr = sprintf('\\textbf{%1.0f}',valUb);
  else
      valUbStr = sprintf('%1.0f',valUbKbr);        
  end
