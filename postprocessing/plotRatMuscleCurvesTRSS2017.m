...
%%
  ...
  plotProps(idx).xticklabels = {num2str(round(plotProps(idx).xticks(1,1),3)),...
                                num2str(round(plotProps(idx).xticks(1,2),3)),...
                                num2str(round(plotProps(idx).xticks(1,3),3)),...
...
%  plotProps(idx).xticklabels = {'$$-1$$','0','$$1$$'};
  ...
  %from createFiberActiveForceLength.m
  plotProps(idx).xlim = ...
    (muscleModel.curves.fiberForceLengthCurve.xEnd +[-0.02,0.02]).*scaleLength;
  ...
  plotProps(idx).xticks      = ...
      [round(muscleModel.curves.fiberForceLengthCurve.xEnd(1,1),2),...                                
        1,...
        round(muscleModel.curves.fiberForceLengthCurve.xEnd(1,2),2)] .* scaleLength;
...
%                   '$$\ell^{T12}+\ell^{Igp}(f)+\ell^{PEVK}(f)+\ell^{Igd}$$'};  
  ...
  plotProps(idx).title  = {'C. Titin segment elongation model','vs. Trombitas et al. 1998 Fig. 5'};  
  ...
  %%
    if(updateTitinPlotsOnly==0)  
      figure(fig_pubCurves);  
      subplotCELength = reshape(subPlotPanel(1,1,:),1,4);
      subplot('Position',subplotCELength);  
        ...
        for i=1:1:length(activeForceLengthData)
            ...
            plot(activeForceLengthData(i).x ,...
                 activeForceLengthData(i).y,...
                 activeForceLengthData(i).mark,...
                 'Color',activeForceLengthData(i).color,...
                 'MarkerFaceColor',activeForceLengthData(i).MarkerFaceColor,...
                 'MarkerSize',markerSize,...
                 'LineWidth',plotProps(idx).lineWidth,...
                 'DisplayName',activeForceLengthData(i).label);
            hold on;
        end
        ...
        if(flag_addFpeCurves==1)
          for i=1:1:length(passiveForceLengthData)
              ...
              plot(passiveForceLengthData(i).x,...
                   passiveForceLengthData(i).y,...
                   passiveForceLengthData(i).mark,...
                   'Color',passiveForceLengthData(i).color,...
                   'MarkerFaceColor',passiveForceLengthData(i).MarkerFaceColor,...
                   'MarkerSize',markerSize,...
                   'LineWidth',plotProps(idx).lineWidth,...
                   'DisplayName',passiveForceLengthData(i).label);
              hold on;
          end
        end

        flag_addFlFittingData=1;
        ...
        if(flag_addFpeCurves==1)
          ...
          for i=1:1:length(fittingData.fpe)
            plot(fittingData.fpe(i).x .* lopt,...
                 ...
                 'MarkerSize',3,...
                 'LineWidth',0.5,...
                 'DisplayName',sourceTypes{i});
            hold on;
          end
    
          plot( curveSampleTitin.x.*(2*scaleLength), ...
          ...
          plot( curveSampleTitin.x.*(2*scaleLength), ...
                ...
                '-','Color',cs.blue,...
                'LineWidth',plotProps(idx).lineWidth,...
                'DisplayName','$$f^{2}(\ell^M)$$ (passive)');
          hold on;      
  
          plot( curveSampleTwoSegmentTitinActive.x.*(2*scaleLength),...
          ...
          plot( curveSampleTwoSegmentTitinActive.x.*(2*scaleLength),...
                ...
                '-','Color',cs.red,...
                'LineWidth',plotProps(idx).lineWidth,...
                'DisplayName','$$f^{2}(\ell^M)$$ (active)');
          hold on;            
    
          
          flag_addfpeNCurve=0;
          if(flag_addfpeNCurve==1)
            curveSampleFPE = calcBezierYFcnXCurveSampleVector(...
                          muscleModel.curves.('fiberForceLengthCurve'), ...
                          200,plotProps(idx).domain);
        
            plot(curveSampleFPE.x .* scaleLength, ...
              curveSampleFPE.y,...
              '-','Color',[1,1,1],...
                  'LineWidth',plotProps(idx).lineWidth*2);
            hold on;    
        ...
        legend('Position',(subplotCELength+[0,-0.675,0,0]));
  ...
  %%
    if(updateTitinPlotsOnly==0)  
      figure(fig_pubCurves);    
      subplotCEVelocity = reshape(subPlotPanel(1,2,:),1,4);
      subplot('Position',subplotCEVelocity);  
        ...
        flag_addFvFittingData=1;
        if(flag_addFvFittingData==1)
          assert(length(fittingData.fv)==1);
          for i=1:1:length(fittingData.fv)
            plot(fittingData.fv.x,...
                 fittingData.fv.y,...
                 'o',...
                 'Color',[0,0,0],...
                 'MarkerFaceColor',[0,0,0],...
        ...
        plot(0,...
             1,...
             '.','Color',plotProps(idx).lineColor,'HandleVisibility','off');
        hold on;            

        text(0.05,-0.01, 'Lengthening','FontSize',8,...
             'VerticalAlignment','bottom');
        hold on;
        text(-0.05,-0.01, 'Shortening','FontSize',8,...
        ...
        xticks(plotProps(idx).xticks);
        if(~isempty(plotProps(idx).xticklabels))
          xticklabels(plotProps(idx).xticklabels);
        end
        yticks(plotProps(idx).yticks);
        yticklabels(plotProps(idx).yticklabels);
...
%       hold on;    
        for i=1:1:length(passiveForceLengthData)
            plot(passiveForceLengthData(i).x,...
                 ...
                 passiveForceLengthData(i).mark,...
                 'Color',passiveForceLengthData(i).color,...
                 'MarkerFaceColor',passiveForceLengthData(i).color,...
                 'MarkerSize',2,...
                 'LineWidth',plotProps(idx).lineWidth,...
                 'DisplayName',passiveForceLengthData(i).label);
            hold on;
  ...
  if(updateTitinPlotsOnly==0)
      ...
      xTxt = interp1(curveSampleTwoSegmentTitinActive.y+curveSampleECMHalf.y,...
                                  curveSampleTwoSegmentTitinActive.x*(2*scaleLength), yTxt);
  ...
  if(flag_fibrilModel==0)
      ...
      xTxt = interp1(curveSampleTitin.y+curveSampleECMHalf.y,...
                                  curveSampleTitin.x*(2*scaleLength), yTxt);
  ...
  if(updateTitinPlotsOnly==0)
      ...
      text(normFiberLengthAtOneNormPassiveForce*scaleLength,...
           0,...
...
%       hold on;  
  ...
  if(flag_fibrilModel==0)
      ...
      xTxt = curveSampleECMHalf.x(z,1)*scaleLength;
  ...
  xTxt = curveSampleTitin.x(z,1)*scaleLength;
  ...
  plot( curveSampleProximalTitin.x .* scaleLength,...
...
%        'LineWidth',plotProps(idx).lineWidth);
  ...
  yTxt =  0.8;%(1-lambdaECM)*(0.5);
  xTxt = interp1( curveSampleProximalTitin.y, curveSampleProximalTitin.x.*scaleLength, yTxt);
...
%       'Color',colorProximal);      
  ...
  plot( curveSampleDistalTitin.x .* scaleLength,...
...
%   hold on;  
  ...
  yTxt = 0.8;
  xTxt = interp1( curveSampleDistalTitin.y, curveSampleDistalTitin.x .* scaleLength, yTxt);  
  xTxt = xTxt + 0.05.*(max(curveSampleDistalTitin.x)-min(curveSampleDistalTitin.x)).*scaleLength;
  ...
  % Igp + PEVK force-length
  %%    
  if(flag_make2by2Plot==0)
    figure(fig_pubCurves);  
    subplotTrombitasFigure5 = reshape(subPlotPanel(1,3,:),1,4);
