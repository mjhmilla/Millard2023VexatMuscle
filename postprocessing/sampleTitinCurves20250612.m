...
function [curveSampleECMHalf, curveSampleTitin, curveSampleTitinActive,...
  ...
  sampleTitinCurves20250612(sampleVectorHalfLength,...
                            modelParameters)
                                

forceLengthECMHalfCurve                       =modelParameters.curves.forceLengthECMHalfCurve                     ;     
forceLengthProximalCurve                      =modelParameters.curves.forceLengthProximalTitinCurve               ;       
forceLengthProximalInverseCurve               =modelParameters.curves.forceLengthProximalTitinInverseCurve        ;      
forceLengthDistalCurve                        =modelParameters.curves.forceLengthDistalTitinCurve                 ;      
forceLengthDistalInverseCurve                 =modelParameters.curves.forceLengthDistalTitinInverseCurve          ;             
forceLengthIgpCurve                           =modelParameters.curves.forceLengthIgPTitinCurve                    ;   
forceLengthIgpInverseCurve                    =modelParameters.curves.forceLengthIgPTitinInverseCurve             ;          
forceLengthPevkCurve                          =modelParameters.curves.forceLengthPevkTitinCurve                   ;    
forceLengthPevkInverseCurve                   =modelParameters.curves.forceLengthPevkTitinInverseCurve            ;           
forceLengthIgdCurve                           =modelParameters.curves.forceLengthIgDTitinCurve                    ;   
forceLengthIgdInverseCurve                    =modelParameters.curves.forceLengthIgDTitinInverseCurve             ;          
normLengthZToT12                              =modelParameters.sarcomere.ZLineToT12NormLengthAtOptimalFiberLength ;          
normLengthProximalSegmentAtOptimalFiberLength =modelParameters.sarcomere.IGPNormLengthAtOptimalFiberLength        ;                                        
normLengthPEVKAtOptimalFiberLength            =modelParameters.sarcomere.PEVKNormLengthAtOptimalFiberLength       ;                                
normLengthIgdFixed                            =modelParameters.sarcomere.IGDFixedNormLengthAtOptimalFiberLength   ;     
titinModelType                                =modelParameters.sarcomere.titinModelType                           ;          
normLengthTitinActinBondMinimum               =modelParameters.sarcomere.normLengthTitinActinBondMinimum          ;
...
for i=1:1:length(sampleVectorHalfLength)
  ...
  lceN = 2*(lPH + lDH + normLengthZToT12 + normLengthIgdFixed);

  if(lceN <= normLengthTitinActinBondMinimum)
    lPHActive = lPH;
  end
  lDHActive = 0.5*lceN- (lPHActive + normLengthZToT12 + normLengthIgdFixed);
  ...
  curveSampleTwoSegmentTitinActive.x(i,1)      = lceN*0.5;
  curveSampleTwoSegmentTitinActive.y(i,1)      = ...
    calcBezierYFcnXDerivative(lDHActive,forceLengthDistalCurve,0);
  curveSampleTwoSegmentTitinActive.dydx(i,1)   = ...
    calcBezierYFcnXDerivative(lDHActive,forceLengthDistalCurve,1);
  curveSampleTwoSegmentTitinActive.d2ydx2(i,1) = ...
    calcBezierYFcnXDerivative(lDHActive,forceLengthDistalCurve,2);
  curveSampleTwoSegmentTitinActive.d3ydx3(i,1) = ...
    calcBezierYFcnXDerivative(lDHActive,forceLengthDistalCurve,3);   
