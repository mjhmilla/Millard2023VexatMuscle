    ...
    for idxActivation = 1:1:length(activation)      
      for i=1:1:length(amplitudeMM)        
        for j=1:1:length(bandwidthHz)
          ...
          [cpsd_Gxy,cpsd_Fxy] = cpsd(x,y,[],[],[],sampleFrequency,'onesided');
          [cpsd_Gxx,cpsd_Fxx] = cpsd(x,x,[],[],[],sampleFrequency,'onesided');
          [cpsd_Gyy,cpsd_Fyy] = cpsd(y,y,[],[],[],sampleFrequency,'onesided');
          
          assert(length(Fxy)==length(Fxx) && length(Fxy) == length(Fyy));

          freqSimData.coherenceSq(:,idx) = ( abs(cpsd_Gxy).*abs(cpsd_Gxy) ) ./ (cpsd_Gxx.*cpsd_Gyy) ;
          freqSimData.coherenceSqFrequency(:,idx) = cpsd_Fxy;
