...
This repository contains the Matlab prototype for the VEXAT muscle model described in Millard et al. Please see the eLife version for the version of record, and the bioRxiv version for the preprint. The contents of both versions are identical though the eLife version includes the eLife assessment text and links to the public reviews of the paper. The model is named after the viscoelastic (VE) crossbridge (X) active-titin (AT) elements contained in the model
