...
%%
%Sticky spring titin curves
%  :Here it is assumed that a point, Q, on the PEVK segment attaches to actin. 
%   Here the proximal elastic curve is formed by lumping the proximal Ig 
%   segment with the piece of the PEVK segment that is proximal to Q. Next, 
%   the distal elastic curve is formed by lumping the piece of the PEVK segment
%   that is distal to Q with the distal Ig segment.
%%
flag_useElasticIgD        = 1;
...
flag_useHumanIgDGeometry  = 0;
flag_createTwoSidedCurves = 0;
%The two sided curve option is in place to cover this special situation with
%the sticky spring model:
%  The CE is activated at a long length, attaching a point on the PEVK 
%  segment to actin. Next the CE is shortened so much that the myosin tip
%  passes the attachement point. In simulation this will slow up as a negative
%  l2 length. The normal one sided curves go to zero at negative lengths, but
%  in this situation the stretching section of titin should apply a force to
%  mysoin, impeding its motion. I've never seen this situation occur in 
%  simulation but its here in case.
%
%  The better option might be to treat the elastic function as a function of
%  length only, and then to use the geometry of the model to choose the 
%  of the force applied to myosin.
...
[normMuscleCurves.forceLengthProximalTitinTwoSidedCurve,...
 normMuscleCurves.forceLengthProximalTitinInverseTwoSidedCurve,...
 normMuscleCurves.forceLengthDistalTitinTwoSidedCurve,...
 normMuscleCurves.forceLengthDistalTitinInverseTwoSidedCurve] ...
          = createTitinCurves2022( normMuscleCurves.fiberForceLengthCurve,...
                                   forceLengthCurveSettings,...
                                   normMuscleCurves.forceLengthECMHalfCurve,...
                                   sarcomereProperties,...
...
[normMuscleCurves.forceLengthProximalTitinCurveTwoSidedHuman,...
 normMuscleCurves.forceLengthProximalTitinInverseCurveTwoSidedHuman,...
 normMuscleCurves.forceLengthDistalTitinCurveTwoSidedHuman,...
 normMuscleCurves.forceLengthDistalTitinInverseCurveTwoSidedHuman] ...
          = createTitinCurves2022( normMuscleCurves.fiberForceLengthCurve,...
                                   forceLengthCurveSettings,...
                                   normMuscleCurves.forceLengthECMHalfCurve,...
                                   sarcomereProperties,...
...
%This curve is used to create the plot that slows the movement of the prox Ig/PEVK
%boundary, and the distal Ig/PEVK boundary to compare the movement with 
%Trombitas et al. 1994.                                
flag_useElasticIgD=0;
flag_useHumanIgDGeometry=1;
...
%
% Of course this is not enough: the WLC actually needs new coefficients 
% (persistence lengths and contour lengths) every time an Ig domain unfolds.
%
